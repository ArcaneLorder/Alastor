...
static size_t zip_entry_mark(struct zip_t* zip,
    ...
    for (i = 0; i < n; ++i) {
        ...
        entry_mark[i].file_index = (size_t)-1;
...
static size_t zip_index_next(mz_uint64* local_header_ofs_array,
    size_t cur_index) {
    size_t new_index = 0, i;
...
static size_t zip_sort(mz_uint64* local_header_ofs_array, size_t cur_index) {
    size_t nxt_index = zip_index_next(local_header_ofs_array, cur_index);
...
static int zip_index_update(struct zip_entry_mark_t* entry_mark,
    size_t last_index, size_t nxt_index) {
    size_t j;
...
static int zip_entry_finalize(struct zip_t* zip,
    ...
    for (i = 0; i < n; ++i) {
        ...
        size_t index = zip_sort(local_header_ofs_array, i);
...
static size_t zip_entry_set(struct zip_t* zip,
...
static size_t zip_file_move(MZ_FILE* m_pFile, const mz_uint64 to,
...
static size_t zip_files_move(MZ_FILE* m_pFile, mz_uint64 writen_num,
...
static size_t zip_entries_delete_mark(struct zip_t* zip,
    struct zip_entry_mark_t* entry_mark,
    int entry_num) {
    ...
    return (size_t)deleted_entry_num;
}
...
static int _zip_entry_open(struct zip_t* zip, const char* entryname,
    ...
    */
    ...
    if (pzip->m_zip_mode == MZ_ZIP_MODE_READING) {
        zip->entry.index = (size_t)mz_zip_reader_locate_file(
            pzip, zip->entry.name, NULL,
            case_sensitive ? MZ_ZIP_FLAG_CASE_SENSITIVE : 0);
        if (zip->entry.index < (size_t)0) {
            err = ZIP_ENOENT;
            goto cleanup;
        }
    ...
    zip->entry.index = (size_t)zip->archive.m_total_files;
...
size_t zip_entry_index(struct zip_t* zip) {
    if (!zip) {
        // zip_t handler is not initialized
        return (size_t)ZIP_ENOINIT;
    }
...
size_t zip_entry_read(struct zip_t* zip, void** buf, size_t* bufsize) {
    mz_zip_archive* pzip = NULL;
    mz_uint idx;
    size_t size = 0;
    ...
    pzip = &(zip->archive);
    if (pzip->m_zip_mode != MZ_ZIP_MODE_READING ||
        zip->entry.index < (size_t)0) {
        // the entry is not found or we do not have read access
        return (size_t)ZIP_ENOENT;
    }

    idx = (mz_uint)zip->entry.index;
    if (mz_zip_reader_is_file_a_directory(pzip, idx)) {
        // the entry is a directory
        return (size_t)ZIP_EINVENTTYPE;
    }
...
size_t zip_entry_noallocread(struct zip_t* zip, void* buf, size_t bufsize) {
    mz_zip_archive* pzip = NULL;
    ...
    if (!mz_zip_reader_extract_to_mem_no_alloc(pzip, (mz_uint)zip->entry.index,
        ...
        return (size_t)ZIP_EMEMNOALLOC;
    }

    return (size_t)zip->entry.uncomp_size;
}
...
size_t zip_entries_total(struct zip_t* zip) {
    if (!zip) {
        // zip_t handler is not initialized
        return ZIP_ENOINIT;
    }

    return (size_t)zip->archive.m_total_files;
}

size_t zip_entries_delete(struct zip_t* zip, char* const entries[],
    size_t len) {
    size_t n = 0;
    size_t err = 0;
...
size_t zip_Stream_copy(struct zip_t* zip, void** buf, size_t* bufsize) {
    size_t n;
